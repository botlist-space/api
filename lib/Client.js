...
class Client {
	...
	constructor(options) {
		...
		if (!(options.userToken instanceof String)) throw new TypeError('User token in options object must be a string');
	 ...
	 */
	getAllBots(page) {
		if (typeof page !== 'undefined' && !(page instanceof Number)) throw new TypeError('Page must be a number');
		if (typeof page !== 'undefined' && page < 1) throw new TypeError('Page number cannot be negative or zero');
		return new Promise((resolve, reject) => {
			snekfetch
	...
	postServerCount(count) {
		if (!(count instanceof Number) && !(count instanceof Array)) throw new TypeError('Server count is not a number or shards array');
		return new Promise((resolve, reject) => {
