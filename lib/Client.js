...
const ClientOptions = require('./Structures/ClientOptions.js');
...
class Client {
	/**
	 * @param {ClientOptions} [options={}] An object with client options.
	 */
	constructor(options = {}) {
		options = Object.assign(ClientOptions, options);
		if (Array.isArray(options) ? true : options !== Object(options)) throw new TypeError('Options must be an object');
		if (options.id !== null && !(options.id instanceof String)) throw new TypeError('ID in options object must be a string');
		if (options.userToken === null && options.botToken === null) throw new ReferenceError('The userToken/botToken in options must be present.');
		if (options.userToken !== null && !(options.userToken instanceof String)) throw new TypeError('User token in options object must be a string');
		if (options.botToken !== null && !(options.botToken instanceof String)) throw new TypeError('Bot token in options object must be a string');
