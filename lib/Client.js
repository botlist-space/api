...
const BotStatistics = require('./Structures/Statistics/Bots.js');
const HTTPError = require('./Structures/Error.js');
const Collection = require('./Structures/Collection.js');
const Statistics = require('./Structures/Statistics.js');
const Bot = require('./Structures/Bot.js');
const Upvote = require('./Structures/Upvote.js');
const User = require('./Structures/User.js');
const GenericResult = require('./Structures/GenericResult.js');
...
class Client {
	...
	getUpvotes(idsOnly = false) {
		...
		return new Promise((resolve, reject) => {
			const collection = new Collection();
			snekfetch
				.get(`${this._baseURL}/bots/${this._id}/upvotes?ids=${idsOnly ? 'true' : 'false'}`, { headers: { Authorization: this._token } })
				.then(body => {
	...
	postServerCount(count) {
		...
		return new Promise((resolve, reject) => {
			...
			snekfetch
				.post(`${this._baseURL}/bots/${this._id}`, { data: data, headers: { Authorization: this._token } })
				.then(result => {
	 ...
	 */
	test() {
		return console.log('Want to use me? Simply replace test() with another function. Fool.');
	}
}
