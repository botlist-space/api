...
const getUserReviews = (id: string, token: string | null = null, page = 1, count = 16, sortBy = 'title', sortDirection: 'ascending' | 'descending' = 'ascending'): Promise<PaginatedResponse<Bot>> => {
	assert(typeof id === 'string', `Expected 'id' to be a 'string', got '${typeof id}'`);
	assert(id.length > 0, `Expected id.length > 0, got ${id.length}`);
	assert(typeof token === 'string' || token === null, `Expected 'token' to be a 'string' or null, got '${typeof token}'`);
	if (token !== null) assert(token.length > 0, `Expected token.length > 0, got ${token.length}`);
	...
	return superagent
		.get(`${BASE_URL}/users/${id}/reviews?${params.toString()}`)
		.set('User-Agent', `discordlist.space Library v${version}`)
		.set('Authorization', token || 'unauthorized')
		.then((result) => result.body);
};

const getUserLists = (id: string, token: string | null = null, page = 1, count = 16, sortBy = 'name', sortDirection: 'ascending' | 'descending' = 'ascending'): Promise<PaginatedResponse<Bot>> => {
	assert(typeof id === 'string', `Expected 'id' to be a 'string', got '${typeof id}'`);
	assert(id.length > 0, `Expected id.length > 0, got ${id.length}`);
	assert(typeof token === 'string' || token === null, `Expected 'token' to be a 'string' or null, got '${typeof token}'`);
	if (token !== null) assert(token.length > 0, `Expected token.length > 0, got ${token.length}`);
	...
	return superagent
		.get(`${BASE_URL}/users/${id}/lists?${params.toString()}`)
		.set('User-Agent', `discordlist.space Library v${version}`)
		.set('Authorization', token || 'unauthorized')
		.then((result) => result.body);
};

export { getUser, getUserBots, getUserServers, getUserReviews, getUserLists };